...
//        1。查询商品(数量，价格)
        for (OrderDetail orderDetail : orderDTO.getOrderDetailList()) {
            ...
            // CproductInfo productInfo = cproductservice.findOne(orderDetail.getProductId());
            if (productInfo == null) {
...
//        5.扣库存
    ...
    //创建衣物清洗订单
    @Transactional
    public OrderDTO createCloth(OrderDTO orderDTO) {
        String orderId = KeyUtil.genUniqueKey();
        BigDecimal orderAmount = new BigDecimal(BigInteger.ZERO);
//        1。查询商品(数量，价格)
        for (OrderDetail orderDetail : orderDTO.getOrderDetailList()) {
...
//            TODO
    ...
    //判断订单状态
        if (!orderDTO.getOrderStatus().equals(OrderStatusEnum.NEW.getCode())){
            log.error(" [完结订单] 更新失败，orderMaster={}", orderDTO.getOrderId());
            throw new SellException(ResultEnum.ORDER_STATUS_ERROR);
        }
    //修改订单状态
        orderDTO.setOrderStatus(OrderStatusEnum.FINIFSH.getCode());
        OrderMaster orderMaster = new OrderMaster();
        BeanUtils.copyProperties(orderDTO,orderMaster);
        OrderMaster updateResult = orderMasterRepository.save(orderMaster);
        if (updateResult == null){
